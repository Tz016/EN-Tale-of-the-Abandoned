...
const totaldata=await window.record();
  ...
  document.getElementById('result').textContent = 'In this test, approximately 40% of the artworks you viewed are found in heaps of garbage, the rest are works in the art market, and the average price of those works is around 251349.25$.';

  document.getElementById('TFQuestions').textContent = `You accurately picked out ${correctTF} piece(s) which are either abandoned works or works with the highest market value; on the slider questions your estimation of the prices were ${SliderQuestion[0]}$, ${SliderQuestion[1]}$, ${SliderQuestion[2]}$, in reality the prices of the 3 artworks are ${sliderv[0]}$, ${sliderv[1]}$, ${sliderv[2]}$ respectively. You accurately estimated the price for ${correctslider}piece of artwork. (A fluctuation of +_500 is counted as accurate) Your overall accuracy is ${percen}%~ 🐦`
  document.getElementById('others').textContent=`Currently, this test has been completed ${totaldata[0]}times, on average, ${totaldata[1]} pieces of abandoned artwork or works with the highest market value can be accurately picked out. The average price of the 3 artworks from the slide question are ${totaldata[4]}$, ${totaldata[5]}$, ${totaldata[6]}$ respectively, and the average number of accurate estimates is ${totaldata[2]} respectively, and the average number of accurate estimates is ${totaldata[3]}% ^o^`
